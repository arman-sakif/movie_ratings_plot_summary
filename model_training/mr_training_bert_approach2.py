...
print(df['rating_class'].isnull().values.any())
...
minority = final_df[final_df.label == 0]
majority_1 = final_df[final_df.label == 1]
majority_2 = final_df[final_df.label == 2]

# Downsample majority class 1
majority_1_downsampled = resample(majority_1,
                                  replace=False,    # sample without replacement
                                  n_samples=len(minority),     # to match minority class
                                  random_state=42) # reproducible results
# Downsample majority class 2
majority_2_downsampled = resample(majority_2,
                                  replace=False,    # sample without replacement
                                  n_samples=len(minority),     # to match minority class
                                  random_state=42)

# Combine minority class with downsampled majority class
df_downsampled = pd.concat([minority, majority_1_downsampled, majority_2_downsampled])

# Display new class counts
print(df_downsampled.label.value_counts())

final_df = df_downsampled.copy()
final_df = final_df.sample(frac=1, random_state=42).reset_index(drop=True)

final_df.tail(10)

# Split the data into 70% training and 30% testing
final_df, test_df = train_test_split(final_df, test_size=0.3, random_state=42)  # random_state for reproducibility
...
# Save the model
model.save_pretrained(os.path.join("..", "trained_models", "trained_BERT_v4"))
